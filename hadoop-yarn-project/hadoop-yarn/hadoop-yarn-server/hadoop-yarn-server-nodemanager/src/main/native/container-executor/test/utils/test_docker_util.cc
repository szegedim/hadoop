...
namespace ContainerExecutor {
  ...
  TEST_F(TestDockerUtil, test_check_mount_permitted) {
    const char *permitted_mounts[] = {"/etc", "/usr/bin/cut", "/tmp/", "^/usr/local/.*$", NULL};
    ...
    test_data.push_back(std::make_pair<std::string, int>("//usr/local/bin/", 1));
    test_data.push_back(std::make_pair<std::string, int>("/usr/local/Cellar/protobuf@fake", -1));
  ...
  TEST_F(TestDockerUtil, test_normalize_mounts) {
    const int entries = 5;
    const char *permitted_mounts[] = {"/home", "/etc", "/usr/bin/cut", "^/dev/nvidia.*$", NULL};
    const char *expected[] = {"/home/", "/etc/", "/usr/bin/cut", "^/dev/nvidia.*$", NULL};
  ...
  TEST_F(TestDockerUtil, test_set_devices) {
    ...
    std::string container_executor_cfg_contents = "[docker]\n"
        "  docker.privileged-containers.registries=hadoop\n"
        "  docker.allowed.devices=/dev/test-device,/dev/device2,^/dev/nvidia.*$,^/dev/gpu-uvm.*$";
    std::vector<std::pair<std::string, std::string> > file_cmd_vec;
    file_cmd_vec.push_back(std::make_pair<std::string, std::string>(
